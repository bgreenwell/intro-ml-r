...
```{r pizza, echo=FALSE, out.width="90%"}
knitr::include_graphics("figures/pizza.png")
```
...
* Started at `r anicon::cia("figures/effo-logo.png", animate = FALSE)` in April of 2018

* BS/MS/PhD in Applied Statistics

* `r anicon::faa("r-project", animate = "pulse")` enthusiast `r anicon::cia("https://emojis.slackmojis.com/emojis/images/1450694616/220/bananadance.gif?1450694616", animate = FALSE)`

* Matlab dethusiast `r anicon::cia("https://emojis.slackmojis.com/emojis/images/1483822352/1572/gross.gif?1483822352", animate = FALSE)`

* Maker of `r icon::fa("r-project")` `r emo::ji("package")`s

* [UC Analytics Summit](https://github.com/koalaverse/AnalyticsSummit18)
...
Nature functions to associate $\boldsymbol{X}$ with $\boldsymbol{Y}$:
...
* Can we predict what $\boldsymbol{Y}$ will be for future $\boldsymbol{X}$?

* How does nature associate $\boldsymbol{Y}$ with $\boldsymbol{X}$?

]

???

Source: Statistical Modeling: The Two Cultures
...
roundhouse::kick("When people tell you machine learning models are just black boxes",
                 type = 2, width = 40, size = 25, fps = 5)
```

[Source](https://bradleyboehmke.github.io/CinDay-RUG-IML-2018/slides-source.html#1)
...
.font125[Complex tree (test error = 0%):]
...
knitr::include_graphics("figures/spam.jpg")
```
...
```{r bootstrap-sample, echo=FALSE, out.width="100%"}
knitr::include_graphics("figures/bootstrap-sample.png")
```
...
```{r fig-bagged-trees, echo=FALSE, out.width="70%"}
...
```{r bagging-1, echo=FALSE, fig.width=6, fig.asp=0.618, out.width="100%"}
# Simulate some sine wave data
set.seed(1112)  # for reproducibility
x <- seq(from = 0, to = 2 * pi, length = 500)
y <- sin(x) + rnorm(length(x), sd = 0.5)
plot(x, y, col = adjustcolor("black", alpha.f = 0.5))
lines(x, sin(x))
legend("topright", legend = "True function", lty = 1, inset = c(0.1, 0.01),
       box.col = "transparent", cex = 0.6)
```
...
```{r bagging-2, echo=FALSE, fig.width=6, fig.asp=0.618, out.width="100%"}
# Fit a single regression tree
ctrl <- list(cp = 0, minbucket = 5, maxdepth = 30)
fit <- rpart(y ~ x, control = ctrl)
pred <- predict(fit)
plot(x, y, col = adjustcolor("black", alpha.f = 0.5))
lines(x, sin(x))
...
legend("topright", legend = lgnd, col = c("black", cols[1L]), 
       lty = 1, inset = c(0.1, 0.01), box.col = "transparent", cex = 0.6)
```
...
```{r bagging-3, echo=FALSE, fig.width=6, fig.asp=0.618, out.width="100%"}
...
for (i in 1:nsim) {
  fit <- rpart(y[id[, i]] ~ x[id[, i]], control = ctrl)
  pred_mat[, i] <- predict(fit)
  lines(x[id[, i]], pred_mat[, i], 
        col = adjustcolor(cols[2L], alpha.f = 0.25))
}
lines(x, sin(x))
lines(x, pred, col = cols[1L], lwd = 2)
...
```{r bagging-4, echo=FALSE, fig.width=6, fig.asp=0.618, out.width="100%"}
plot(x, y, col = adjustcolor("black", alpha.f = 0.5))
for (i in 1:nsim) {
  lines(x[id[, i]], pred_mat[, i], 
        col = adjustcolor(cols[2L], alpha.f = 0.25))
...
legend("topright", legend = lgnd, col = c("black", cols[c(1, 2, 6)]), lty = 1, 
       inset = c(0.1, 0.01), box.col = "transparent", cex = 0.6)
```
...
.center.bold[Check out R's [machine learning task view](https://cran.r-project.org/web/views/MachineLearning.html)!]
...
### <span style="color:MediumSeaGreen">GBMs:</span>
