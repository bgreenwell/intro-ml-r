---
title: "Introduction to Machine Learning in `r icon::fa('r-project')` `r anicon::cia('https://emojis.slackmojis.com/emojis/images/1450319453/122/carlton.gif?1450319453', animate=FALSE, size=1, border=F)`"
subtitle: "Analytics Connect '18"
author: "Brandon M. Greenwell<br>Enable the Science/Data Scientist<br>[`r anicon::faa('github', animate='vertical')` Fork me on GitHub](https://github.com/bgreenwell/intro-ml-r)<br>[`r anicon::faa('twitter', animate='float', rtext='@bgreenwell8')`](https://twitter.com/bgreenwell8)"
output:
...
background-image: url(https://images-na.ssl-images-amazon.com/images/I/71iK6wQfd4L._UL1500_.jpg?fifu)
background-size: 70%
...
* .purple[Machine learning]: construcintg algorithms that learn from data

* .purple[Statistical learning]: emphasizes statistical models and the assessment of uncertainty

* .purple[Data science]: applying mathematics, statistics, machine learning, engineering, etc. to extract knowledge form data (isn't this applied statistics `r emo::ji("thinking")`)

]

.full-width.content-box-purple.font150.center[Similar terms with slightly different emphases!]
...
.full-width.content-box-purple.font150.center[This talk is about *supervised machine learning*: building models from data that predict an outcome **Y** using a collection of features **X**]

--

.font150[

* Many flexible tools for making predictions from data

* They are not magick! They require **good data** and proper validation
...
```{r fundamentals, echo=FALSE, out.width="80%"}
...
It is possible to "over tune" your models to maximize performance on the training data. Such models will not generalize well to new data. We call this process *overfitting*.
...
tst <- tibble::tibble(
  x = runif(n = 1000, min = -2, max = 2),
  y = rnorm(n = 1000, mean = 1 + 2*x + x^2, sd = 1)
)
p <- ggplot(df, aes(x, y)) + 
...
# Two goals in model development
...
<!-- We *typically* split the data into two non-overlapping sets: -->

* The .bold[training set] is used to fit the models

* Tthe .bold[validation set] is used to estimate prediction error for model selection
...
.full-width.content-box-yellow.font150.center[What if we don't have enough data for training, validation, and testing?]
...
* Prone to over fitting 
    ...
    - Need to regularize or otherwise reduce the number fetures
...
.full-width.content-box-yellow.font150.center[GzLMs, .bold.red[tree-based methods], GBMs, and network models (like DNNs)]
...
```{r sklearn-chart, echo=FALSE, out.width="90%"}
...
```{r apm-chart, echo=FALSE, out.width="90%"}
...
# A handy new rule for mushrooms
...
```{r, echo=FALSE, fig.width=4, fig.asp=1, out.width="100%", animation.hook="gifski", dev="png"}
# Load required packages
library(gganimate)
...
# df <- data.frame(tree = seq_len(spam_bag$ntree), 
#                  error = spam_bag$test$err.rate[, "Test"])
...
ggplot(df, aes(x = num_trees, y = error, color = type)) +
  geom_path() +
